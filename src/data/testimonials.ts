import { Testimonial } from '@/types';

/** Algemene reviews zoals getoond op de homepage. */
export const GENERAL_TESTIMONIALS: Testimonial[] = [
  {
    id: 'g1',
    name: 'Anoniem',
    text: 'Toen ik op zoek was naar een orthomoleculair therapeut en ging zoeken in de omgeving, kwam ik uit bij Danique. Na een uitgebreide intake en bloedtest ben ik de adviezen op gaan volgen, onder andere bepaalde voeding vermijden en aanvulling met supplementen voor tekorten. Nu zijn we inmiddels een tijdje verder, ongeveer 3 maanden en ik voel me beter. Na twee maanden merkte ik verbetering doordat ik bijv. wat meer energie had en minder opgeblazen was. Voorlopig blijf ik nog even doorgaan. Danique houdt goed contact, geeft tussendoor tips en neemt de tijd om te luisteren en mee te denken!',
    rating: 5,
    image: '',
  },
  {
    id: 'g2',
    name: 'Anoniem',
    text: 'Nadat ik al langere tijd verschillende klachten had en ik vastliep in de reguliere zorg ben ik verder op zoek gegaan wat me zou kunnen helpen. Mijn klachten waren op mentaal en fysiek vlak. Achteraf gezien een combi van burn-out, overgang, schildklier en fibromyalgie. Orthomoleculaire therapie sprak me erg aan omdat er gewerkt wordt vanuit een holistische visie. Zo kwam ik bij Danique Kwakman uit. Ze heeft me enorm geholpen om inzicht in mijn klachten te krijgen en wat ik eraan kon gaan doen. Door aanpassingen in mijn voeding en leefstijl voel ik me al stukken beter. Na een operatie had ze ook tips hoe ik me weer sneller fit kon gaan voelen. Vooral niet twijfelen maar gewoon contact met Danique Kwakman opnemen en ervaar het zelf!',
    rating: 5,
    image: '',
  },
  {
    id: 'g3',
    name: 'Anoniem',
    text: 'Ik kwam bij Danique met ernstige menstruatieklachten en een zeer onregelmatige cyclus. We hebben samen naar de opties gekeken; nu ben ik veel regelmatiger en ik heb veel minder klachten. Ik zou Danique echt aanraden als je naar een fijn traject met veel persoonlijke aandacht en een vriendelijke aanpak op zoek bent!',
    rating: 5,
    image: '',
  },
  {
    id: 'g4',
    name: 'Anoniem',
    text: 'In eerste instantie ging ik bij Danique langs voor mijn acne. Door middel van een bloedtest zijn we achter een aantal intoleranties gekomen! Met die info heeft Danique mij tips and tricks qua voeding maar ook advies over supplementen gegeven. We zijn nu een aantal maanden verder en niet alleen mijn huid ziet er stralender uit, ook voel ik mij een stuk fitter en ben ik me meer bewust van wat ik kan doen op het moment dag ik mij futloos voel. Ik raad het zeker aan om langs te gaan. Bedankt Danique voor je hulp!',
    rating: 5,
    image: '',
  },
  {
    id: 'g5',
    name: 'Anoniem',
    text: 'Ik ben drie dagen geleden weer ongesteld geworden, super blij mee 😊.',
    rating: 5,
    image: '',
  },
  {
    id: 'g6',
    name: 'Anoniem',
    text: 'Met mijn huid gaat het beter, ik merk dat ik geen nieuwe puistjes/ontstekingen erbij krijg. Wel is mn huid nog herstellende. Dus helemaal egaal is het nog niet haha.',
    rating: 5,
    image: '',
  },
];

/** Ervaringen zoals aangeleverd voor het 1:1 bloedsuikertraject. */
export const BLOEDSUIKER_TESTIMONIALS: Testimonial[] = [
  {
    id: 'b1',
    name: 'Anoniem',
    text: "Danique!! Ik voel me zoooveel stabieler. Heb sinds onze samenwerking niet meer duizeligheid ervaren tijdens het sporten en m'n lijf reageert zoveel rustiger op eten. We hebben wat dingen aangepast in hoe en wat we eten thuis en ik merk echt een wereld van verschil. Ik vond de begeleiding zo fijn, je hebt zoveel kennis!! En heel fijn dat het een wat korter traject was, direct inzichten en vanuit de calls met jou weer dingen om mee aan de slag te gaan. Ik ben heeeel blij dat ik dit heb gedaan!",
    rating: 5,
    image: '',
  },
  {
    id: 'b2',
    name: 'Anoniem',
    text: 'Goed! Ik moest even schakelen met weer ontbijt maken, ik voel me veel meer verzadigd gedurende de dag. Minder behoefte om tussendoor te eten, of soms helemaal niet.',
    rating: 5,
    image: '',
  },
  {
    id: 'b3',
    name: 'Anoniem',
    text: 'Ik was altijd moe en futloos en was veel wakker in de nacht. Totdat ik Danique tegen kwam en zij vertelde over je bloedsuiker spiegel. Zelf had zij ervaren dat haar bloedsuikerspiegel daalde in de nacht en dat zij daarom vaker wakker werd. Toen zij haar energie shift begon, heb ik mij ook gelijk aangemeld en al snel werd het duidelijk dat dit bij mij ook het geval was. Zij heeft mij geholpen met het stabiliseren van mijn bloedsuikerspiegel door middel van de juiste voeding en ik heb daardoor geen suikerdips meer in de nacht.\nIk ging beter slapen en heb veel meer inzicht gekregen in wat mijn bloedsuikerspiegel met mij doet.\nDanique geeft goede adviezen en ze weet waar ze het over heeft.',
    rating: 5,
    image: '',
  },
  {
    id: 'b4',
    name: 'Anoniem',
    text: 'Veel minder snaaien, normaal had ik snel zin in zoet en zout eigenlijk at ik helemaal niet goed. Altijd maar kleine beetjes. Nu geeft dat in mn hoofd ook veel meer rust.',
    rating: 5,
    image: '',
  },
  {
    id: 'b5',
    name: 'Anoniem',
    text: "Hi Danique, even kort berichtje. Merk al wel echt wat suiker met mijn lichaam doet😅.. afgelopen weekend (met verjaardagen) natuurlijk wel hier en daar dingen gegeten met suikers etc. Had gister zo'n last van mijn maag / opgezette buik. Dus is nu eigenlijk nog makkelijker om er dus vanaf te blijven dus dat is mooi om te zien! Hoe het allemaal werkt!",
    rating: 5,
    image: '',
  },
  {
    id: 'b6',
    name: 'Anoniem',
    text: "Ja m'n energie is top! Ik heb ook geen cravings meer",
    rating: 5,
    image: '',
  },
];
